...
public class MySurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    int colors[]={Color.GREEN, Color.BLUE,
            Color.BLACK, Color.MAGENTA, Color.LTGRAY,
            Color.CYAN, Color.RED, Color.DKGRAY, Color.YELLOW};
    float masx[] = new float[2];
    float masy[] = new float[2];

    class DrawThread extends Thread {
        boolean flag = true;
        Paint my_print = new Paint();
        Paint my_print_1 = new Paint();
        Paint my_print_2 = new Paint();
        ...
        public void run() {
            ...
            super.run();
            my_print.setColor(Color.YELLOW);
            while (flag) {
                Canvas c = holder.lockCanvas();
                w = c.getWidth();
                h = c.getHeight();
                my_print_1.setColor(colors[i]);
                my_print_2.setColor(colors[j]);
                if (i == j) {
                    flag = false;
                }
                if (c != null) {
                    c.drawColor(Color.WHITE);
                    c.drawCircle(x, y, r, my_print_1);
                    c.drawCircle(x1, y1, r, my_print_2);
                    c.drawRect(left, top, right, bottom, my_print);
                    if (xf == true)
                        x += 25;
                    else
                        x -= 25;
    ...
    public static class GameActivity extends Activity {
        @Override
